...
game.player_turn_color="red";/*set ton prwto paixth*/
for(let i = 0 ; i<6; i++){
...
if (window.location.hash === '#change') {
    ...
    document.querySelector(".init-players form").style.justifyContent="center";
...
function playComputer(){
    ...
    var x=computerMove_X();
    var y=computerMove_Y();
...
function computerMove_X() {
    return rows[Math.floor(Math.random() * rows.length)];
} 

function computerMove_Y() {

    return colums[Math.floor(Math.random() * colums.length)];
} 
